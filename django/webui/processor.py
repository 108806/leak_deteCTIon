    ...
    for obj in objects:
        ...
        try:
            with transaction.atomic():
                scrap_file = ScrapFile.objects.create(name=obj.object_name, sha256=file_hash, size=FILE_SIZE)
                del FILE_SIZE
                    ...
                    if len(line) > 512: # Edge Cases with file in file
                        ...
                        for nested_line in nested_lines:
                            ...
                            try:
                                ...
                                print(f"Error saving line from file {obj.object_name}: {nested_line}, Error: {e}")
                                traceback.print_exc()
                    LINES_TOTAL += 1
                    try:
                        BreachedCredential.objects.create(
